...
export const getCardInfos = async (req, res) => {
  ...
  try {
    if (otherUserId) {
      ...
      logger.info(
        `/routes/cards/cardInfo 폴더 getCardInfos함수, get, cardId : ${cardId} 다른 유저 카드 조회!`
      )
    ...
    logger.info(
      `/routes/cards/cardInfo 폴더 getCardInfos함수, get, cardId : ${cardId} 내 카드 조회!`
    )
    res.status(200).send(cardInfo)
  } catch (err) {
    logger.error(
      '/routes/cards/cardInfo 폴더 getCardInfos함수, get, err : ',
      err
    )
...
const getCardInfo = async (userId, cardId) => {
  ...
  try {
    ...
    logger.info(
      `/routes/cards/cardInfo 폴더 getCardInfo함수, get, data : ${data}!`
    )
    return data
  } catch (err) {
    connection?.release()
    logger.error(
      '/routes/cards/cardInfo 폴더 getCardInfo함수, get, err : ',
      err
    )
